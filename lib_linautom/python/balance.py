...
#			print "Fin normal"
	...
	def close(self):
		...
		if self.port.startswith("/dev"):
			...
			return self.com.send("\x03"+self.fin_de_ligne) # fonctionnement avec pond raspberry pi
			self.com.close()
...
class Sartorius(BalanceCom):
	...
	def decodage(self, reponce):
		...
		self.poids = 0
		if reponce_nettoye[1] == "H" or reponce_nettoye[0] == "HH":
			self.erreur = "SURCHARGE"
			raise BalanceSurCharge
		elif reponce_nettoye[1] == "L" or reponce_nettoye[0] == "LL":
...
class Balance(object):
	...
	def lecture(self, unit="mg"):
		...
		if self.sleep_continu_read == None:
			...
			return {"poids" : self.poids_unit, "stabilitee" : self.stabilitee}
		else:
